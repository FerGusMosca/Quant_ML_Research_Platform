...
def show_commands():
    ...
    print("#1-TrainMLAlgos [SeriesCSV] [from] [to]")
    print("#2-RunPredictionsLastModel [SeriesCSV] [from] [to]")
    print("#3-EvalBiasedTradingAlgo [Symbol] [SeriesCSV] [from] [to] [Bias]")
...
def process_train_ml_algos(cmd_param_list,str_from,str_to):
...
def process_commands(cmd):
    ...
    if cmd_param_list[0]=="TrainMLAlgos":
        params_validation("TrainMLAlgos", cmd_param_list, 4)
        process_train_ml_algos(cmd_param_list[1], cmd_param_list[2], cmd_param_list[3])
