...
class VainillaChunkGenerator:
    ...
    def __init__(self,logger):
        self.logger=logger
    def chunk(self,text: str, target_tokens=180, overlap_tokens=40, k=3):
        try:
            if self.logger:
                self.logger.do_log("[MSC] 🔍 Starting Multi-Stage Chunking...", 1)

            # ===== Stage 1: Sentence segmentation =====
            try:
                sentences = nltk.sent_tokenize(text)
                if self.logger: self.logger.do_log(f"[MSC] 📌 Sentences extracted: {len(sentences)}", 2)
            except Exception as e:
                if self.logger: self.logger.do_log(f"[MSC] ❌ Sentence split failed: {e}", 0)
                return []

            if len(sentences) == 0:
                if self.logger: self.logger.do_log("[MSC] ❌ No sentences found.", 0)
                return []
            ...
            try:
                ...
                if self.logger: self.logger.do_log(f"[MSC] ❌ Embeddings failed: {e}", 0)
                return []

            # ===== Stage 3: K-means clustering (semantic grouping) =====
            try:
                real_k = min(k, len(embeddings))
                if self.logger: self.logger.do_log(f"[MSC] 🎯 Clustering k={real_k}", 2)

                kmeans = KMeans(n_clusters=real_k, n_init=5)
                labels = kmeans.fit_predict(embeddings)
            except Exception as e:
                if self.logger: self.logger.do_log(f"[MSC] ❌ K-means failed: {e}", 0)
                return []
            ...
            for lab, sents in clusters.items():
                if self.logger:
                    self.logger.do_log(f"[MSC] 📦 Processing cluster {lab} ({len(sents)} sentences)", 2)
            ...
            if self.logger:
                self.logger.do_log(f"[MSC] ✅ Final chunks: {len(final_chunks)}", 1)

            return final_chunks

        except Exception as e:
            if self.logger:
                self.logger.do_log(f"[MSC] ❌ UNEXPECTED ERROR in chunk(): {e}", 0)
            return []
