...
class RandomForestModelCreator(BaseModelCreator):
    ...
    def train_random_forest_daily(self, training_series_df, model_output, symbol, classif_key,
        ...
        try:
            ...
            # Save full bundle (model + encoder + scaler)
            model_bundle = {
                "model": final_model,
            ...
            print(f"Random Forest model bundle saved to {model_output}")
    ...
    def test_RF_scalping(self, symbol, test_series_df, model_to_use, price_to_use="close",
                         ...
                         threshold=0.5, preloaded_model=None,label_encoder=None,draw_statistics=False):
        """
        ...
        if preloaded_model is None:
            model, _, _ = self.__load_rf_model_bundle__(model_to_use)
        else:
