...
class ExecutionReportDTO(BaseModel):
    ...
    currency: str
    broker: str
    text: Optional[str] = None          # ← rejection reason / free text from exchange
    ...
    def parse_ord_type(value):
        """Castea el valor de OrdType del JSON al Enum OrdType."""
        try:
            if isinstance(value, int):
                value = chr(value)
    ...
    def parse_ord_status(value):
        """Castea el valor de OrdStatus del JSON al Enum OrdStatus."""
        try:
            if isinstance(value, int):
                value = chr(value)
    ...
    def from_execution_report(data: dict, broker_id: str) -> "ExecutionReportDTO":
        ...
        return ExecutionReportDTO(
            ...
            broker=broker_id,
            text=data.get("Text") or data.get("text"),   # ← capture rejection reason
        )